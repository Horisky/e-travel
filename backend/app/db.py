...
async def get_weekly_usage(user_id: str, week_start: str) -> int:
    pool = await get_pool()
    if pool is None:
        return 0
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "select count from user_usage where user_id=%s and week_start=%s",
                (user_id, week_start),
            )
            row = await cur.fetchone()
            if not row:
...
async def increment_weekly_usage(user_id: str, week_start: str) -> None:
    pool = await get_pool()
    if pool is None:
        return
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                """
                insert into user_usage (user_id, week_start, count)
                values (%s, %s, 1)
...
async def get_total_usage(user_id: str) -> int:
    pool = await get_pool()
    if pool is None:
        return 0
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "select coalesce(sum(count), 0) from user_usage where user_id=%s",
                (user_id,),
            )
            row = await cur.fetchone()
            return int(row[0] or 0)
